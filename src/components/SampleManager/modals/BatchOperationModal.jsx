...
const BatchOperationModal = ({ open, onCancel, taskId, onSuccess, module = 'workflow' }) => {
    ...
    const cascaderOptions = useMemo(() => {
        ...
        return taskSamples.map(s => {
            ...
            const isNotSelfCreated = s.creator_id === null || s.creator_id === undefined || s.creator_name === null || s.creator_name === undefined;
            const isOperationRestricted = ['itemCreate', 'itemDelete', 'methodCreate', 'methodDelete'].includes(operation.value);
            const isSelfRestrictDisabled = (module === 'department' || module === 'testing') && isNotSelfCreated && isOperationRestricted;

            const isSampleDisabled = (operation.value === 'processCreate' && s.type !== 0) || isSelfRestrictDisabled;
            ...
            const itemNodes = (s.items || []).map(item => {
                ...
                if (operation.levels === 2) {
                    return { 
                        ...
                        value: itemId, 
                        isLeaf: true,
                        disabled: isProcessingUnfinished || (operation.value === 'processCreate' && s.type !== 0) || isSelfRestrictDisabled
                    };
                }
                ...
                return { 
                    ...
                    disabled: (operation.levels === 3 && subNodes.every(n => n.disabled)) || isSelfRestrictDisabled
                };
            ...
            return { 
                ...
                disabled: isSampleDisabled || (operation.levels > 1 && itemNodes.every(n => n.disabled))
            };
                    ...
                    <div className="space-y-4 p-4 bg-emerald-50/30 border border-emerald-100 rounded-3xl">
                        ...
                        >
                            <Select 
                                ...
                                options={module === 'workflow' ? departments.map(d => ({ label: d.name, value: d.id })) : users.map(u => ({ label: u.nickname || u.name, value: u.id }))} 
                                className="w-full" 
                            />
                        </Form.Item>
