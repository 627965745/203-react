...
import { Tag, Switch, message, Button, Space, Modal, Form, Input, Popconfirm } from "antd";
...
const ProcessingMethodList = () => {
    ...
    const [optionModal, setOptionModal] = useState({ visible: false, method: null, option: null });
    const [form] = Form.useForm();

    const handleSwitchEnabled = async (record, checked) => {
        try {
            const res = await updateProcessingMethod({ ...record, enabled: checked ? 1 : 0 });
    ...
    const handleOptionSubmit = async (values) => {
        try {
            ...
            const api = isEdit ? updateProcessingOption : createProcessingOption;
    ...
    const columns = [
        ...
        },
        {
            ...
            dataIndex: "name",
            width: "25%",
            render: (text) => <span className="font-bold text-indigo-600 text-[15px]">{text}</span>
        },
        {
            title: "加工选项",
            key: "optionsList",
                ...
                <Space size={[4, 8]} wrap>
                    {record.options?.map(opt => (
                        <Tag 
                            ...
                            onClick={() => {
                                setOptionModal({ visible: true, method: record, option: opt });
                                form.setFieldsValue({ value: opt.value, enabled: opt.enabled === 1 });
                            }}
                        >
                            {opt.value}
                        </Tag>
                    ...
                    <Tag 
                        onClick={() => {
                            setOptionModal({ visible: true, method: record, option: null });
    ...
    return (
        <>
            ...
            <Modal
                ...
                onCancel={() => setOptionModal({ visible: false, method: null, option: null })}
                cancelText="取消"
                okText="确认"
                onOk={() => form.submit()}
                width={400}
            ...
            >
                ...
                >
                    <Form.Item label="所属方法" className="mb-4">
                        <Tag color="cyan" className="m-0">{optionModal.method?.name}</Tag>
                    </Form.Item>
                    <Form.Item 
                        label="选项值" 
                        name="value" 
                    ...
                    >
                        <Input placeholder="例如: 平口, 螺纹..." maxLength={255} />
                    </Form.Item>
