...
import { Modal, Form, Input, InputNumber, Button, Space, message, Select, DatePicker, Divider, List, Card, Tooltip } from "antd";
import { PlusOutlined, DeleteOutlined, InfoCircleOutlined } from "@ant-design/icons";
import dayjs from "dayjs";
import { prepareReferenceMaterial, readReferenceMaterial } from "../../api/referenceMaterial";
...
const PrepareModal = ({ visible, onCancel, onSuccess }) => {
    ...
    const fetchMaterials = async () => {
        try {
            const res = await readReferenceMaterial({ page: 0, rows: 1000 });
            if (res.data.status === 0 || res.data.code === 0) {
                // The read API returns { rows, total } or just an array
    ...
    const handleFinish = async (values) => {
        try {
            ...
            const payload = {
                ...values,
                confirmed_at: values.confirmed_at ? values.confirmed_at.format("YYYY-MM-DD") : undefined,
                expiring_at: values.expiring_at ? values.expiring_at.format("YYYY-MM-DD") : undefined,
            };
    ...
    return (
        <Modal
            ...
            className="top-4"
        >
            <Form form={form} layout="vertical" onFinish={handleFinish} className="max-h-[75vh] overflow-y-auto pr-4 pl-2 py-4">
                        ...
                        {(fields, { add, remove }) => (
                            <>
                                {fields.map(({ key, name, ...restField }) => {
                                    ...
                                    const selectedMaterial = materialOptions.find(m => m.id === selectedId);
                                    const unitLabel = selectedMaterial?.unit || "ml/g";

                                    return (
                                        <div key={key} className="flex gap-4 items-start mb-3 bg-white p-3 rounded shadow-sm border border-slate-100">
                                            <div className="flex-1">
                                                <Form.Item
                                                    {...restField}
                                                    name={[name, 'id']}
                                                    className="m-0"
                                                    rules={[{ required: true, message: '请选择来源物质' }]}
                                                >
                                                    <Select
                                                        showSearch
                                                        ...
                                                        options={materialOptions.map(m => ({
                                                            label: `${m.id}. ${m.name} (${CategoryMap[m.category] || '--'}/${StageMap[m.stage] || '--'}/${PhysicalStateMap[m.physical_state] || '--'}) - 余量: ${m.remaining}${m.unit || ''}`,
                                                            value: m.id
                                                        }))}
                                                    />
                                                </Form.Item>
                ...
                <div className="grid grid-cols-3 gap-x-4">
                    <Form.Item name="name" label="新试剂名称" rules={[{ required: true }]} className="col-span-2">
                    ...
                    <Form.Item name="physical_state" label="物理形态" rules={[{ required: true }]}>
                        <Select options={PhysicalStateOptions} />
                    </Form.Item>
                    <Form.Item name="medium_type_id" label="介质类型" rules={[{ required: true }]}>
                        <Select
                            showSearch
                            optionFilterProp="label"
                            options={mediumOptions.map(m => ({ label: m.name, value: m.id }))}
                        />
                    </Form.Item>
                    ...
                    <Form.Item name="remaining" label="余量" rules={[{ required: true }]}>
                        <InputNumber className="w-full" min={0} />
                    </Form.Item>
                    <Form.Item name="alert_threshold" label="报警阈值" rules={[{ required: true }]}>
                        <InputNumber className="w-full" min={0} />
                    </Form.Item>
                    ...
                    <Form.Item name="confirmed_at" label="定值日期">
                        <DatePicker className="w-full" placeholder="请选择定值日期"/>
                    </Form.Item>
