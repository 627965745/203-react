...
const FieldDrawer = ({ visible, onClose, cover, onSuccess }) => {
    ...
    const [editingField, setEditingField] = useState({ enabled: 1, type: 0 });

    useEffect(() => {
        if (visible && cover) {
            setFields(cover.fields || []);
        }
    }, [visible, cover?.id, cover?.fields]);
    ...
    const handleAdd = () => {
        setEditingField({ enabled: 1, type: 0 });
    ...
    const handleModalOk = async () => {
        ...
        try {
            ...
            if (res.data.code === 0 || res.data.status === 0) {
                ...
                // Ideally we'd refetch, but we'll try to update local state optimistically or mock it.
                // Or if it returns the updated field, we can merge it. If not we create a mock ID for display until actual refetch.
