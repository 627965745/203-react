...
import { fieldDeleteReportTable, fieldCreateReportTable, fieldUpdateReportTable } from '../../api/reportTable';
...
const FieldDrawer = ({ visible, onClose, tableRecord, onSuccess }) => {
    ...
    const [editingField, setEditingField] = useState({ enabled: 1, type: 0, sort: 0 });

    useEffect(() => {
        if (visible && tableRecord) {
            const sortedFields = [...(tableRecord.fields || [])].sort((a, b) => a.sort - b.sort);
    ...
    const handleModalOk = async () => {
        ...
        try {
            ...
            const apiCall = payload.id ? fieldUpdateReportTable : fieldCreateReportTable;
    ...
    return (
        <>
            <Drawer
                title={tableRecord ? `数据表字段管理 - ${tableRecord.name}` : '数据表字段管理'}
                size="large"
                placement="right"
                onClose={onClose}
                open={visible}
