...
import {
    fieldDeleteTestMethod,
    fieldCreateTestMethod,
    fieldUpdateTestMethod,
    fieldTestMethod,
    readTestMethod,
} from "../../api/testMethod";
...
const FieldDrawer = ({ visible, onClose, methodRecord, onSuccess }) => {
    ...
    useEffect(() => {
        if (visible && methodRecord) {
            const sortedFields = [...(methodRecord.fields || [])].sort(
                (a, b) => a.sort - b.sort,
            );
    ...
    const columns = [
        ...
        },
        {
            ...
            width: 120,
            render: (text) => (
                <code className="bg-gray-100 p-1 px-2 rounded text-xs">
                    {text}
                </code>
            ),
        },
        ...
        },
        {
            ...
            render: (type) => {
                ...
                return (
                    <Tag color={config.color} className="m-0 border-none">
                        {config.text}
                    </Tag>
                );
            },
        },
        {
            ...
            width: 220,
            render: (_, record) => {
                const config = SOURCE_TYPES[record.source_type] || {
                    text: "未知",
                };
                let detail = "";
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space orientation="vertical" size="small" className="text-xs">
                    <Tag
                        color={record.is_required === 1 ? "error" : "default"}
                        className="m-0"
                    >
